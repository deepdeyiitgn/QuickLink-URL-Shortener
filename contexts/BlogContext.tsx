...
export const BlogProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const fetchPosts = useCallback(async () => {
        setLoading(true);
        try {
            const fetchedPosts = await api.getBlogPosts(auth?.currentUser?.id);
    ...
    const updatePost = async (update: any): Promise<BlogPost> => {
    ...
    const toggleLike = async (postId: string) => {
        ...
        const updatedPost = await updatePost({ postId, action: 'toggle_like', userId: auth.currentUser.id });
        setPosts(prev => prev.map(p => p.id === postId ? updatedPost : p));
    };
    
    const addComment = async (postId: string, commentData: Omit<Comment, 'id' | 'createdAt'>) => {
        const updatedPost = await updatePost({ postId, action: 'add_comment', comment: commentData, userId: auth?.currentUser?.id });
        setPosts(prev => prev.map(p => p.id === postId ? updatedPost : p));
    };

    const incrementShares = async (postId: string) => {
        const updatedPost = await updatePost({ postId, action: 'increment_share' });
        setPosts(prev => prev.map(p => p.id === postId ? updatedPost : p));
    };
    ...
    const incrementView = async (postId: string) => {
        ...
        try {
            await updatePost({ postId, action: 'increment_view' });
        } catch (error) {
    ...
    const deletePost = async (postId: string) => {
        ...
        try {
            await apiFetch('/api/blog', {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ postId, userId: auth.currentUser.id }),
            });
    ...
    const togglePinPost = async (postId: string) => {
        ...
        try {
            await updatePost({ postId, action: 'toggle_pin', userId: auth.currentUser.id });
            await fetchPosts();
        } catch (error) {
    ...
    const approvePost = async (postId: string) => {
        ...
        try {
            const updatedPost = await updatePost({ postId, action: 'approve_post', userId: auth.currentUser.id });
            setPosts(prev => prev.map(p => p.id === postId ? updatedPost : p));
        } catch (error) {
