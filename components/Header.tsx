...
import { LogoIcon, UserIcon, CrownIcon, ShieldCheckIcon, MenuIcon } from './icons/IconComponents';
...
const Header: React.FC<HeaderProps> = ({ currentView }) => {
    ...
    const getLinkClass = (path: string) => {
        return `text-sm font-medium transition-colors ${currentView === path ? 'text-brand-primary' : 'text-gray-300 hover:text-brand-primary'}`;
    };
    ...
    return (
        <>
                    ...
                    <div className="flex h-16 items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <nav className="hidden md:flex items-center space-x-6">
                                <a href="/shortener" className={getLinkClass('shortener')}>URL Shortener</a>
                                <a href="/qr-generator" className={getLinkClass('qr-generator')}>QR Generator</a>
                                <a href="/qr-scanner" className={getLinkClass('qr-scanner')}>QR Scanner</a>
                                <a href="/blog" className={getLinkClass('blog')}>Blog</a>
                                <a href="/faq" className={getLinkClass('faq')}>FAQ</a>
                                {currentUser && (
                                    <a href="/api" className={getLinkClass('api')}>Developer API</a>
                                )}
                            </nav>
                        </div>
                        ...
                        <div className="hidden md:flex items-center gap-4">
                            {currentUser ? (
                                <div className="flex items-center gap-4">
                                    {currentUser.isAdmin ? (
                                        <a href="/owner" className="flex items-center gap-2 text-sm font-medium text-yellow-400 hover:text-yellow-300 transition-colors">
                                    ...
                                    {currentUser.subscription && currentUser.subscription.expiresAt > Date.now() && (
                                        <button onClick={() => auth?.openSubscriptionModal?.()} title="You are a subscriber!" className="text-green-400">
                        ...
                        <div className="md:hidden">
                            <button onClick={() => setMobileMenuOpen(true)} className="p-2 text-gray-300 hover:text-white" aria-label="Open menu">
            ...
            <MobileMenu isOpen={isMobileMenuOpen} onClose={() => setMobileMenuOpen(false)} currentView={currentView} />
        </>
    );
};
