...
const UrlShortener: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const finalAlias = alias.trim() || Math.random().toString(36).substring(2, 8);
            ...
            const newUrl: ShortenedUrl = {
                ...
                createdAt: Date.now(),
                expiresAt: currentUser?.canSetCustomExpiry ? null : expiresAt,
                userId: currentUser?.id || null,
            };

            await urlContext.addUrl(newUrl);
    ...
    const getExpiryMessage = () => {
        if (currentUser?.canSetCustomExpiry) return 'Links created by you are permanent.';
        if (expiryDays === 365) return `Your premium links will last for ${expiryDays} days.`;
