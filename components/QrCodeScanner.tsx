...
const QrCodeScanner: React.FC = () => {
        ...
        qrScanner.start({ facingMode: "environment" }, config, onScanSuccess, () => {})
            .catch((err: any) => {
                ...
                if (err.name === 'NotAllowedError') {
                    errorMessage = 'Camera access was denied. Please grant camera permissions in your browser settings to use the scanner.';
                } else if (err.name === 'NotFoundError') {
                    errorMessage = 'No camera was found on this device. Please try uploading an image instead.';
                } else if (err.name === 'NotReadableError') {
                    errorMessage = 'The camera is currently in use by another application or could not be started. Please close other apps and try again.';
                } else {
                    errorMessage = `Camera Error: ${err.message}. Please ensure you have given camera permissions.`;
                }
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setScanError("Scan failed: No QR code found. Please try a different image. For best results, use a clear, high-contrast picture with the QR code centered and well-lit.");
            setScanMode('idle');
        } finally {
