...
const CANVAS_SIZE = 512; // Define higher resolution for QR code generation
...
const QrCodeGenerator: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const qrOptions = {
            width: CANVAS_SIZE, // Generate a higher resolution QR code for sharpness
            margin: 2,
            errorCorrectionLevel: logoSrc ? 'H' : 'Q', // Use highest error correction with a logo
        ...
        QRCode.toCanvas(canvas, payload, qrOptions).then(() => {
            if (logoSrc) {
                ...
                logoImg.src = logoSrc;
                logoImg.onload = () => {
                    // Calculate logo size to be smaller for better scannability
                    const logoSize = canvas.width * 0.18; 
                    const logoX = (canvas.width - logoSize) / 2;
                    const logoY = (canvas.height - logoSize) / 2;
                    
                    // Add a clear "quiet zone" around the logo
                    const padding = 8;
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                    ...
                    <div className="flex flex-col items-center justify-center bg-black/20 p-6 rounded-lg border border-white/10">
                        <div className="relative w-[300px] h-[300px] flex items-center justify-center bg-brand-dark rounded-md">
                            ...
                            <canvas ref={canvasRef} width={CANVAS_SIZE} height={CANVAS_SIZE} className="hidden"></canvas>
                        </div>
