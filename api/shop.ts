...
async function handleUpdateProduct(req: any, res: any, db: any) {
    const { productId, adminId, productData } = req.body;
    if (!await isAdmin(db, adminId)) return res.status(403).json({ error: 'Unauthorized' });

    // Sanitize data, don't allow changing ID, createdAt etc.
    const { id, createdAt, ...updateFields } = productData;

    await db.collection('products').updateOne({ id: productId }, { $set: updateFields });
...
export default async function handler(req: any, res: any) {
    ...
    try {
        ...
        if (req.method === 'PUT') {
            if (type === 'product') return handleUpdateProduct(req, res, db);
        }
